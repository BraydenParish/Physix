...
import { runVisibilityTick } from '../src/tick.js';
...
describe('Firewall LOS visibility', () => {
  test('players see each other with clear LOS', () => {
    ...
    const result = runVisibilityTick(
      world,
      { simStepsToRun: 1, simulateStep: () => world.step() },
      { position: observer.translation() },
  ...
  test('does not step the world inside visibility computation', () => {
    const mockWorld: any = {
      propagateModifiedBodyPositionsToColliders: vi.fn(),
      castRay: () => null,
      step: () => {
    ...
    const result = runVisibilityTick(
      mockWorld,
      { simStepsToRun: 0, simulateStep: () => {} },
      observer,
      targets
    );

    expect(result[0].visible).toBe(true);
    expect(mockWorld.propagateModifiedBodyPositionsToColliders).toHaveBeenCalledTimes(1);
  });
  ...
  test('visibility mirrors wall-hit presence (property)', () => {
    fc.assert(
      fc.property(fc.array(fc.boolean(), { minLength: 1, maxLength: 6 }), (hitFlags) => {
        ...
        const results = runVisibilityTick(
          mockWorld,
          { simStepsToRun: 0, simulateStep: () => {} },
          observer,
          targets
  ...
  test('integration: tick order step -> propagate -> visibility reacts to wall insertion/removal', () => {
    const world = new RAPIER.World({ x: 0, y: 0, z: 0 });
    const ctx = { world, rapier: RAPIER };
    const observer = spawnPlayer(ctx, { x: 0, y: 1, z: 0 });
    const target = spawnPlayer(ctx, { x: 10, y: 1, z: 0 });
    ...
    const wall = spawnWallBox(ctx, { x: 5, y: 1, z: 0 }, { x: 1, y: 2, z: 5 });
    const withWall = run();
    expect(withWall[0].visible).toBe(false);

    world.removeCollider(wall, true);
    const afterRemoval = run();
    expect(afterRemoval[0].visible).toBe(true);
  });

  test('integration: spawn/despawn without stepping still syncs queries once', () => {
    const world = new RAPIER.World({ x: 0, y: 0, z: 0 });
    const ctx = { world, rapier: RAPIER };
    const observer = spawnPlayer(ctx, { x: 0, y: 1, z: 0 });
    const target = spawnPlayer(ctx, { x: 10, y: 1, z: 0 });
    ...
    const run = () =>
      runVisibilityTick(
        world,
        { simStepsToRun: 0, simulateStep: () => {}, bodiesMutatedOutsideStep: true, sceneChanged: true },
        { position: observer.translation() },
        [{ id: 'target', position: target.translation() }]
      );
